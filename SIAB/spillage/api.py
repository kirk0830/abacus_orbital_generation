...
from SIAB.interface.env import _rerun_folder
import unittest
...
def _make_guess(nzeta, folder, jy = True, diagnosis = True):
    ...
    ----------
    nzeta: list[list[int]]
        the number of zeta functions for each l for each orbitals
    folder: str
    ...
    if jy:
        keys = ["nzeta", "diagnosis", "outdir"]
        elem = folder.split("-")[0]
        # because routine pseudopotential have z_val no larger than 40, so we plus 20
        nband = _nband_infer_from_hund(elem, nzeta_max) + min(_elem_z(elem), 20)
        # start ABACUS run in the `folder` to get enough information
    else:
        keys = ["nzeta", "diagnosis", "orb_mat"]
