class AtomSpeciesGeneartor:
    ...
    def __init__(self, 
        ...
        pseudo_name: str
            the pseudopotential name, which is used to generate the orbital
        bessel_nao_rcuts: list
...
""")
    ...
    def build_from_cif(fname: str, scale: float, kspacing: float = -1.0):
        ...
        from SIAB.io.cifparser import read
        import numpy as np
        cif = read(fname)
        labels = cif["_atom_site_type_symbol"]
        ...
        sym_ks = [] # seekpath_result.get('point_coords')
        possible_kpath = [] # seekpath_result.get('path')
    ...
    def build_bravis(bravis: str, scale: float, kspacing: float = -1.0):
        from SIAB.structure.bravis_and_molecule import lookup_acwf_db, lookup_bravis_angles, \
    ...
    def build_molecule(molecule: int, bond_length: float, kspacing: float = -1.0):
        from SIAB.structure.bravis_and_molecule import lookup_molecule
